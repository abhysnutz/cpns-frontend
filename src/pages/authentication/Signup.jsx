...
export default function Signup() {
    ...
    const handleSubmit = async e => {
        ...
        await axios.post(`${import.meta.env.VITE_BACKEND_API_URL}/api/user`,formData)
            .then((response) => {
                if(!response.data.success) return setErrors(response.data.error)
                return navigate('/')
            }).catch((err) => {
            ...
            <div className="mt-8 sm:mx-auto w-full md:w-1/2">
                <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                    ...
                    <form className="space-y-5 " onSubmit={handleSubmit}>
                        ...
                        <div>
                            {/* <button type="submit" @click="signUp" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 items-center"
                                :className="btnDisabled ? 'bg-indigo-300 hover:bg-indigo-300 focus:ring-indigo-100' : 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500'"> */}
