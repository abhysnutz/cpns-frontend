...
export default function Account() {
    ...
    const getData = async () => {
        ...
        setFormData3({"old_password":"","new_password":"","confirm_password":"","update":"password"})
        setName(user.name)
        setEmail(user.email)
    ...
    const handleChange = (e) => {
        ...
        if(name == 'old_password' || name == 'new_password' || name == 'confirm_password'){
    ...
    return (
        <Main Notification={Notification} notifs={notifs} isNotifSuccess={isNotifSuccess}>
            <LeftCol menu={menu} name={name} email={email} changeMenu={changeMenu} />
            <RightCol>
                <div className={menu == 1? 'block' : 'hidden'}>
                    <AccountForm user={user} handleChange={handleChange} email={email} name={name} isSubmit={isSubmit} handleSubmit={handleSubmit} />
                </div>
                <div className={menu == 2? 'block' : 'hidden'}>
                    <ProfileForm handleChange={handleChange} isSubmit={isSubmit} handleSubmit={handleSubmit} phone={phone} birth={birth} education={education} major={major} address={address} provinceId={provinceId} cities={cities}  cityId={cityId} />
                </div>
                <div className={menu == 3? 'block' : 'hidden'}>
                    <PasswordForm handleSubmit={handleSubmit} isSubmit={isSubmit} handleChange={handleChange}/>
                </div>
            </RightCol>
        </Main>
    )
...
const AccountForm = ({handleChange, email, name, isSubmit, handleSubmit}) => {
    return( 
...
const ProfileForm = ({handleChange, isSubmit, handleSubmit, phone, birth, education, major, address, provinceId, cities, cityId }) => {
...
const PasswordForm = ({handleSubmit, isSubmit, handleChange}) => {
    return(
        <form id="update-password" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 gap-4">
                <div>
                    <label className="text-sm font-medium">Password Sekarang:</label>
                    <input type="password" required name="old_password" placeholder="Password Sekarang" className="shadow-sm mt-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md py-3" onChange={handleChange}/>
                </div>
                <div>
                    <label className="text-sm font-medium">Password Baru:</label>
                    <input type="password" required name="new_password" placeholder="Password Baru" className="shadow-sm mt-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md py-3" onChange={handleChange}/>
                </div>
                <div>
                    <label className="text-sm font-medium">Ulangi Password Baru:</label>
                    <input type="password" required name="confirm_password" placeholder="Ulangi Password Baru" className="shadow-sm mt-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md py-3" onChange={handleChange}/>
                </div>
            </div>
            {
                !isSubmit ? 
...
const Main = ({children, notifs, isNotifSuccess, Notification}) => {
    return (
...
function LeftCol({email, menu, name, changeMenu}){
    let avatar = `https://ui-avatars.com/api/?name=${name}&background=6366f1&color=fff`;
        ...
        <div className="xl:col-span-3">
            <aside className="bg-white rounded-md shadow border py-4">
                ...
                <nav aria-label="Tabs" className="space-y-1">
                    <a href="#" className={` ${menu == 1 ? 'bg-blue-50 border-blue-700 text-blue-700 hover:bg-blue-50 hover:text-blue-700' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-800'} group border-l-4 px-3 py-2 flex items-center text-sm font-medium`} onClick={() => changeMenu(1)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className={`${menu == 1 ? 'text-blue-700' : 'text-gray-400'} group-hover:text-blue-700 flex-shrink-0 -ml-1 mr-3 h-5 sm:h-6 w-5 sm:w-6`}>
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2"></path>
                        </svg>
                        <span className="truncate">Akun</span>
                    </a>
                    <a href="#" className={` ${menu == 2 ? 'bg-blue-50 border-blue-700 text-blue-700 hover:bg-blue-50 hover:text-blue-700' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-800'} group border-l-4 px-3 py-2 flex items-center text-sm font-medium`} onClick={() => changeMenu(2)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true" className={`${menu == 2 ? 'text-blue-700' : 'text-gray-400'} group-hover:text-blue-700 flex-shrink-0 -ml-1 mr-3 h-5 sm:h-6 w-5 sm:w-6`}>
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <span className="truncate">Profil</span>
                    </a>
                    <a href="#" className={` ${menu == 3 ? 'bg-blue-50 border-blue-700 text-blue-700 hover:bg-blue-50 hover:text-blue-700' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-800'} group border-l-4 px-3 py-2 flex items-center text-sm font-medium`} onClick={() => changeMenu(3)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true" className={`${menu == 3 ? 'text-blue-700' : 'text-gray-400'} group-hover:text-blue-700 flex-shrink-0 -ml-1 mr-3 h-5 sm:h-6 w-5 sm:w-6`}>
